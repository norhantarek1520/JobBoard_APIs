...
class CategoryController {
  ...
  static createCategory = async (req, res, next) => {
    try {
      ...
      if (result === true) res.status(201).json(" Category added  successfully ")
  ...
  static updateCategory = async (req, res, next) => {
    try {
      ...
      else res.status(404).json(' Faild to update this category')
  ...
  static deleteCategory = async (req, res, next) => {
    try {
      ...
      if (result === true) res.status(204).json(" Category deleted  successfully ")
      else res.status(404).json('Fail to delete this category')
    } catch (error) {
      // Log the error for debugging purposes
      console.error(error);
      // Handle the error appropriately, send an error response to the client
  ...
  static JobsInOneCategory = async (req, res, next) => {
    try {
      ...
      else { res.status(404).json('Theire is No Jobs in this category ')}
